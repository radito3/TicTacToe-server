...
class LobbyManager {
	...
	struct Lobby {
		...
		Lobby(PlayerInfo player1, PlayerInfo player2, unsigned search_id) : player1(player1), player2(player2), search_id(search_id), p1_is_active(false), p2_is_active(false) {}
	};
...
public:
	...
	void create_lobby(PlayerInfo player1, PlayerInfo player2, unsigned lobby_id) {
		active_lobbies.emplace(lobby_id, Lobby(player1, player2, lobby_id));
	}
